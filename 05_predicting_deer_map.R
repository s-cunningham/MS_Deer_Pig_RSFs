...
for (i in 1:length(split_counties)) {
  ...
  # create filename
  filename <- paste0("output/deer_county_preds/deer_pred_", split_counties[[i]]$COUNTYNAME, "_INDmean.tif")
...
## List county rasters
files <- list.files(path="output/deer_county_preds/", pattern="_INDmean.tif$", full.names=TRUE)
...
writeRaster(pred90, "results/predictions/deer_INDrsf_predicted.asc", NAflag=-9999, overwrite=TRUE) # has linear stretch for RAMAs
# additionally save mean predictions as .tif for plotting (both resampled and original 30x30m)
writeRaster(pred90, "results/predictions/deer_INDrsf_predicted_90m.tif", overwrite=TRUE) # has linear stretch for RAMAs
writeRaster(predls, "results/predictions/deer_INDrsf_predicted_linStr_30m.tif", overwrite=TRUE)
writeRaster(pred, "results/predictions/deer_INDrsf_predicted_30m.tif", overwrite=TRUE) 
# plot binned rsf values (log scale)
writeRaster(rc1, "results/predictions/deer_INDrsf_bins_30m.tif", overwrite=TRUE)

...
adj.f <- seq(1.05, 3, by=0.1)
adj.s <- seq(1.05, 1.5, length=length(adj.f))

# Empty array to hold pop count
pig.array <- array(0, dim=c(6,length(Year),length(adj.f)))
pig.array[,1,] <- N0

for (i in 1:length(adj.f)) {
  
  A_adj <- pig.matrix
  A_adj[1, ] <- A_adj[1, ] * adj.f[i] 
  A_adj[4, ] <- A_adj[4, ] * adj.f[i] 
  A_adj[2:3,1:3] <- pig.matrix[2:3,1:3] * adj.s[i] 
  A_adj[5:6,4:6] <- pig.matrix[5:6,4:6] * adj.s[i] 
...
both.incr <- both.incr |>
  as_tibble() |>
  pivot_longer(1:length(adj.f),names_to="increase", values_to="Nt") |>
  arrange(increase)

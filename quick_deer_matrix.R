...
# Rt[i] <- Rmax * exp((-log(Rmax)*Nt)/(K))
# Rmax <- 1.77
# K <- 163702
# N0 <- 162073 + (0.1*162073)
# 
# Rmax * exp((-log(Rmax)*N0)/(K))
...
Fecundity <- c(0, 0.65, 0.76, 0.76, 0.76, 0.76)
N.0<-c(22565,17712,12557,8902,21685,22565,15617,8304,3714,2352)
...
for(i in 1:Sims){
  ...
  ##  Annual stochasticity in survival
  # Females
  deer.matrix[2,1] <- rnorm(1, 0.60, 0.05)
  deer.matrix[3,2] <- rnorm(1, 0.82, 0.05)
  deer.matrix[4,3] <- rnorm(1, 0.63, 0.05)
  ...
  deer.matrix[6,6] <- rnorm(1, 0.49, 0.05)
  # Males
  deer.matrix[8,7] <- rnorm(1, 0.52, 0.05)
  deer.matrix[9,8] <- rnorm(1, 0.82, 0.05)
  deer.matrix[10,9] <- rnorm(1, 0.63, 0.05)
  deer.matrix[11,10] <- rnorm(1, 0.53, 0.05)
  deer.matrix[12,11] <- rnorm(1, 0.44, 0.05)
  deer.matrix[12,12] <- rnorm(1, 0.49, 0.05)
  
  ## Calculate Nt
  for (y in 2:length(Year)){
    deer.array[,y,i]<-deer.matrix%*%deer.array[,y-1,i] #Make sure to multiply matrix x vector (not vice versa)
    
    # Density dependence .... see page 84-85, 200 in RAMAS manual
    Rt <- Rmax * exp((-log(Rmax)*sum(deer.array[,y,i]))/(K))
    
    ## Density dependent adjustment to fecundity
    deer.matrix[1,1:6] <- Fecundity*Rt
    deer.matrix[7,1:6] <- Fecundity*Rt
    
  }
}
# N.median <- apply(apply(deer.array,c(2,3),sum),1,median)
N.median <- apply(apply(deer.array,c(2,3),sum),1,mean)
N.20pct <- apply(apply(deer.array,c(2,3),sum),1,quantile, probs = 0.20)
N.80pct <- apply(apply(deer.array,c(2,3),sum),1,quantile, probs = 0.80)
...
# Looks like this gives complex numbers...?
# ev <- eigen(deer.matrix)
# vectors <- ev$vectors

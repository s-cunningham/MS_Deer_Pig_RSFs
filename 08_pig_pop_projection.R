...
# Set up pig matrix
pig.array <- array(0,dim=c(12,length(Year),Sims))
pig.matrix <- matrix(0,12,12)
...
# Survival
# Females
pig.matrix[c(1,7),1:6] <- Fecundity
pig.matrix[2,1] <- 0.52
pig.matrix[3,2] <- 0.93
# Males
pig.matrix[4,3] <- 0.52
...
for(i in 1:Sims){
  ...
  # Survive & stay
  A_s[6,6] <- rnorm(1, pig.matrix[6,6], s_pct_f)
  # Male survival
  for (s in 1:5) {
    # Survive & go
  ...
  # Survive & stay
  A_s[12,12] <- rnorm(1, pig.matrix[12,12], s_pct_m)

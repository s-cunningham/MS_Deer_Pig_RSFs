...
deer_lower <- c(0.55,          # Neonate survival
                0.5,          # Fawn survival
...
deer_upper <- c(1.6,          # Neonate survival
                1.06,         # Fawn survival
...
# run optimizer
set.seed(1)
deer_opt <- optim(par=runif(14, 0.9, 1.2), fn=objective_fn_deer, method="L-BFGS-B", lower=deer_lower, upper=deer_upper, control = list(trace = 1, maxit=1000))
...
# Fawn survival
A_adj[2,1] <- A_base[2,1]*deer_opt$par[2]
A_adj[8,7] <- A_base[8,7]*deer_opt$par[2]

# Female survival
A_adj[3,2] <- A_base[3,2]*deer_opt$par[3]
A_adj[4,3] <- A_base[4,3]*deer_opt$par[4]
A_adj[5,4] <- A_base[5,4]*deer_opt$par[5]
A_adj[6,5] <- A_base[6,5]*deer_opt$par[6]
A_adj[6,6] <- A_base[6,6]*deer_opt$par[7]

# Male survival
A_adj[9,8] <- A_base[9,8]*deer_opt$par[8]      
A_adj[10,9] <- A_base[10,9]*deer_opt$par[9]    
A_adj[11,10] <- A_base[11,10]*deer_opt$par[10]  
A_adj[12,11] <- A_base[12,11]*deer_opt$par[11] 
A_adj[12,12] <- A_base[12,12]*deer_opt$par[12]

## Fecundity
# Yearlings
A_adj[1,2] <- A_base[1,2]*deer_opt$par[13]
A_adj[7,2] <- A_base[7,2]*deer_opt$par[13]
# Adults
A_adj[1,3:6] <- A_base[1,3:6]*deer_opt$par[13]
A_adj[7,3:6] <- A_base[7,3:6]*deer_opt$par[13]


#### Run population model ####
set.seed(1)
res14 <- run_deer_mod(A_adj, neonate_surv=deer_opt$par[1], theta=deer_opt$par[14], K=1347232, Sims=1000, years=100, ev_sd=0.05, harvest=TRUE, c_dd=c_dd) 
...
# # Calculate for pre-breeding census (fetal counts)
FecundityM <- c(0, R0y*A_base[3,2], R0a*A_base[4,3], R0a*A_base[5,4], R0a*A_base[6,5], R0a*A_base[6,6])*pct_m
FecundityF <- c(0, R0y*A_base[3,2], R0a*A_base[4,3], R0a*A_base[5,4], R0a*A_base[6,5], R0a*A_base[6,6])*pct_f
...
#### Run population model ####
set.seed(1)
res22 <- run_deer_mod(A_adj,neonate_surv=deer_opt$par[1], theta=deer_opt$par[14], K=2090532, Sims=1000, years=100, ev_sd=0.05, harvest=TRUE, c_dd=c_dd) 
...
nsurv_14f <- res14$neonate_surv |>
  mutate(density = "14.3 deer km<sup>-2</sup>",
         stage="Neonate",
         sex="Female")
nsurv_14m <- res14$neonate_surv |>
  mutate(density = "14.3 deer km<sup>-2</sup>",
         stage="Neonate",
         sex="Male")


nsurv_22f <- res22$neonate_surv|>
  mutate(density = "22 deer km<sup>-2</sup>",
         stage="Neonate",
         sex="Female")
nsurv_22m <- res22$neonate_surv|>
  mutate(density = "22 deer km<sup>-2</sup>",
         stage="Neonate",
         sex="Male")

nsurv <- bind_rows(nsurv_14f, nsurv_14m, nsurv_22f, nsurv_22m)
write_csv(nsurv, "results/deer_neonate_survival.csv")
